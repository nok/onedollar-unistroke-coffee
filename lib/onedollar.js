  ...
  OneDollar = (function() {
    ...
    OneDollar.prototype.check = function(points) {
      ...
      for (name in _ref) {
        ...
        if (this.binds[name] !== void 0) {
          space = this.__findBestTemplate(points, template_points);
          if (space < equality) {
            equality = space;
            template = name;
          }
      ...
      if (template !== null) {
        ...
        if (ranking[0].score >= this.CFG.score) {
          args = {
            name: ranking[0].name,
            ...
            path: {
              ...
              centroid: this.___centroid(this.__convert(raw))
            },
            ranking: ranking
          };
    ...
    OneDollar.prototype.start = function(i, point) {
      this.cands[i] = [];
      return this.cands[i].push(point);
    };

    OneDollar.prototype.update = function(i, point) {
      return this.cands[i].push(point);
    };

    OneDollar.prototype.end = function(i, point) {
      this.cands[i].push(point);
      this.check(this.cands[i]);
      return delete this.cands[i];
    };

    OneDollar.prototype._transform = function(points) {
      points = this.__convert(points);
      points = this.__resample(points);
      points = this.__r2Zero(points);
      points = this.__s2Square(points);
      points = this.__t2Origin(points);
      return points;
    };
    ...
    OneDollar.prototype.__resample = function(points) {
      ...
      seperator = (this.___length(points)) / (this.ALGO.parts - 1);
      distance = 0;
      result = [];
    ...
    OneDollar.prototype.__r2Zero = function(points) {
      var centroid, theta;
      centroid = this.___centroid(points);
      theta = Math.atan2(centroid.y - points[0].y, centroid.x - points[0].x);
      return this.___r(points, -theta, centroid);
    };

    OneDollar.prototype.__s2Square = function(points) {
      ...
      return this.___s(points, new Vector(this.ALGO.size / (maxX - minX), this.ALGO.size / (maxY - minY)));
    };

    OneDollar.prototype.__t2Origin = function(points) {
      var centroid;
      centroid = this.___centroid(points);
      return this.___t(points, centroid.mult(-1));
    };

    OneDollar.prototype.__findBestTemplate = function(points, template_points) {
      var a, b, c, d, path_a, path_b, treshold;
      a = this.___rd(-this.ALGO.angle);
      b = this.___rd(this.ALGO.angle);
      treshold = this.___rd(this.ALGO.step);
      c = (1.0 - this.MATH.PHI) * b + this.MATH.PHI * a;
      d = (1.0 - this.MATH.PHI) * a + (this.MATH.PHI * b);
      path_a = this.___getDifferenceAtAngle(points, template_points, c);
      path_b = this.___getDifferenceAtAngle(points, template_points, d);
    ...
    OneDollar.prototype.___getDifferenceAtAngle = function(points, template_points, radians) {
      var centroid;
      centroid = this.___centroid(points);
      points = this.___r(points, radians, centroid);
      return this.___getDifference(points, template_points);
    };

    OneDollar.prototype.___getDifference = function(template, candidate) {
    ...
    OneDollar.prototype.___t = function(points, offset) {
      var point, _i, _len;
      for (_i = 0, _len = points.length; _i < _len; _i++) {
        point = points[_i];
        point.add(offset);
    ...
    OneDollar.prototype.___s = function(points, offset) {
      var point, _i, _len;
      for (_i = 0, _len = points.length; _i < _len; _i++) {
        point = points[_i];
        point.mult(offset);
    ...
    OneDollar.prototype.___r = function(points, radians, pivot) {
    ...
    OneDollar.prototype.___rd = function(degrees) {
